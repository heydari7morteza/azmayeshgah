...
@section('content')
    ...
    <div class="card card-primary">
        ...
        <form role="form" action="{{route('userCheckEditLiquid' , $liquid->id)}}" method="POST" >
            @csrf
            <div class="card-body">
                <div class="form-group has-error">
                    <input id="project_id" type="hidden" class="form-control @error('project_id') is-invalid @enderror" name="project_id" value="@if($liquid->project_id) {{$liquid->project_id}}@endif"  aria-label="Small" aria-describedby="inputGroup-sizing-sm">
                    @error('project_id')
            ...
            <div class="card-footer">
                ...
                @if (Auth::user()->type == 0)
                    <a class="btn btn-danger" href="{{route('userAll_liquids')}}">انصراف</a>
                @else
