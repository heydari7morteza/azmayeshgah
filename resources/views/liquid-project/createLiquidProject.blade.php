...
@section('content')
    ...
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form role="form" action="{{route('checkCreateLiquidProject' , $project->id)}}" method="POST" >
            @csrf
            <div class="card-body">
                <div class="form-group has-error">
                    <input id="project_id" type="hidden" class="form-control @error('project_id') is-invalid @enderror" name="project_id" value="{{$project->id}}" aria-label="Small" aria-describedby="inputGroup-sizing-sm">
                    @error('project_id')
                ...
                <div class="form-group has-error">
                    <input id="user_id" type="hidden" class="form-control @error('user_id') is-invalid @enderror" name="user_id" value="{{Auth::user()->id}}" aria-label="Small" aria-describedby="inputGroup-sizing-sm">
                    @error('user_id')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="card-footer">
                ...
                @if ($project->id)
                    <a class="btn btn-danger" href="{{route('liquidProject' , $project->id)}}">انصراف</a>
                @else
                    <a class="btn btn-danger" href="{{route('userShowLiquids' , Auth::user()->id)}}">انصراف</a>
                @endif
